...
class _PersistentFaceSolver:
    ...
    def __init__(self):
        ...
        self.model.setObjective(
            gp.quicksum(0.0 * self.x[i] for i in range(N_FACES)), GRB.MINIMIZE
        )
        self.model.update()
...
def train_prior():
    ...
    for epoch in range(N_EPOCHS):
        ...
        grads_flat = jnp.concatenate(
            [jnp.ravel(g) for g in jax.tree.leaves(grads)]
        )
        ...
        if grads_finite and loss_finite:
            ...
            n_skipped += 1
            print(f"  epoch {epoch + 1:4d}: ⚠ NaN in "
...
def evaluate_prior(
    ...
    result = env.step(
        Action(face=face, contact_point=contact, angle=angle), n_sim_steps=n_sim_steps
    )
...
def hybrid_finetune(params, env: PushTEnv, seed: int = 9999, save_video: bool = True):
    ...
    print(f"\n  [hybrid final] mean_dist = {final.mean():.4f} m  "
